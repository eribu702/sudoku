...
void Sudoku::check_info(const int y, const int x)
{
	...
	if (grid[y][x].slot < 0 || grid[y][x].slot > 9) {
	...
	const int num = grid[y][x].slot - 1;//to avoid indexing like this: filled[grid[y][x] - 1]
	bool fail = false;
	...
	if (grid[y][x].row->filled[num] == true) {
		...
		std::cout << y + 1 << " has more than one " << grid[y][x].slot << "'s!\n";
		std::exit(0);
	}
